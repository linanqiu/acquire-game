...
- [x] Core stock purchase scenarios covered (7 tests: 6.1-6.3, 6.4, 6.5-6.6, 6.8-6.12, 6.14, 6.15-6.7, extended 10+ turns)
- [x] Stock stepper enforces 0-3 range per chain
- [x] Total purchase limited to 3 shares per turn
- [x] Insufficient funds shows error/disables purchase
- [x] Pool exhaustion reflected in UI (available stock counts checked)
- [x] Screenshots capture purchase flow
...
|---------------|-------------|
| Purchase limits (6.1-6.6) | 10+ turns with actual purchases, track cash changes |
| Constraints (6.7-6.12) | Play until constraint scenario occurs naturally |
| Edge cases (6.13-6.17) | Loop until precondition met (e.g., merger for 6.13) |

### Screenshots Required Per Purchase

- `turn-N-buy-phase-start` - Stock purchase UI appears
- `turn-N-stock-selected-{chain}` - Stepper incremented
- `turn-N-total-cost` - Cost calculation shown
- `turn-N-purchase-confirmed` - After clicking buy
- `turn-N-holdings-updated` - New stock visible
- `turn-N-cash-updated` - Cash decreased

### Verify Actual Changes, Not Just UI

```typescript
// WRONG - just checks UI looks right
await expect(page.getByTestId('stock-stepper-tower')).toBeVisible()

// RIGHT - verify actual state changes
const cashBefore = await getCash(page)
const holdingsBefore = await getHoldings(page, 'tower')

await incrementStepper(page, 'tower')
await confirmPurchase(page)

const cashAfter = await getCash(page)
const holdingsAfter = await getHoldings(page, 'tower')

expect(holdingsAfter).toBe(holdingsBefore + 1)
expect(cashAfter).toBeLessThan(cashBefore)
```

### Track Purchase History

Log every purchase across all turns:
```typescript
const purchases: Array<{turn: number, chain: string, quantity: number, cost: number}> = []
// ... after each purchase
console.log(`Turn ${turn}: Bought ${qty} ${chain} for $${cost}`)
