...
|---------------|-------------|
| Player count (8.1-8.4) | Test actual validation, not just UI state |
| Hand management (8.5-8.8) | Track tile counts across 10+ turns |
| Invalid tiles (8.9-8.14) | Loop until unplayable tile scenario occurs |
| Reconnection (8.15-8.18) | Actually disconnect and verify full state restore |
| Error handling (8.23-8.24) | Trigger real errors, verify graceful handling |

### Reconnection Tests Must Be Real

```typescript
// WRONG - simulating events doesn't test real reconnection
await page.evaluate(() => window.dispatchEvent(new Event('offline')))

// RIGHT - actually close the WebSocket
await page.evaluate(() => {
  // Access the real WebSocket and close it
  const ws = window.__gameWebSocket // Expose this for testing
  ws.close()
})
// Wait for reconnection UI
await expect(page.getByTestId('reconnection-overlay')).toBeVisible()
// Wait for automatic reconnect
await expect(page.getByTestId('reconnection-overlay')).not.toBeVisible({ timeout: 10000 })
// Verify state is fully restored
await verifyGameState(page, expectedState)
```

### Screenshots Required for Edge Cases

For each edge case:
- `precondition` - Setup state before edge case
- `edge-case-triggered` - The unusual state
- `handling` - How the system handles it
- `resolution` - State after handling

### WebSocket Gotchas for Reconnection

See [ST-002 WebSocket Gotchas](./ST-002.md#2-react-strictmode-double-mount-race-condition-the-real-issue) for issues that may affect reconnection tests:
- React StrictMode may cause double reconnection attempts
- Verify `wsRef.current` is properly managed across reconnects
- Test both explicit disconnect and network interruption
