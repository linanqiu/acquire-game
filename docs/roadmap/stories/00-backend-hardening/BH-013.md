...
- [ ] `PlayTileResult` dataclass for `play_tile()` response
- [ ] `FoundChainResult` dataclass for `found_chain()` response
- [ ] `MergerResult` dataclass for merger-related responses
- [ ] `BuyStocksResult` dataclass for `buy_stocks()` response
- [ ] `EndTurnResult` dataclass for `end_turn()` response
- [ ] `EndGameResult` dataclass for `end_game()` response
- [ ] All response classes have `to_dict()` method for serialization
- [ ] Type hints added to all Game method signatures
