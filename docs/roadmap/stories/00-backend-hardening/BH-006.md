...
- [x] `get_tile_playability()` returns dict with each tile's status
- [x] Status includes: `playable`, `reason` (if not playable), `permanent` flag
- [x] Reasons: "would_merge_safe_chains", "would_create_eighth_chain"
- [x] Player state includes `tile_playability` field
- [x] Unit tests for playability detection
