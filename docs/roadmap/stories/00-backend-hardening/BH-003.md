...
- [x] `GameEvent` dataclass with timestamp, event_type, player_id, details
- [x] `Game` class has `_events` list with max 50 recent events
- [x] Events emitted for: tile placement, chain founding, chain expansion, merger, stock disposition, stock purchase, trade, end turn, game over
- [x] `get_public_state()` includes `recent_events` field
- [x] Events are human-readable strings
