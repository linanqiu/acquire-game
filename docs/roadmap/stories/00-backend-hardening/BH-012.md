...
- [ ] Tests for all valid transitions (see transition map)
- [ ] Tests verify invalid transitions raise exceptions
- [ ] Tests for complete turn cycle (PLACE_TILE → BUY_STOCKS → PLACE_TILE)
- [ ] Tests for merger flow (PLACE_TILE → MERGING → STOCK_DISPOSITION → BUY_STOCKS)
- [ ] Tests for founding flow (PLACE_TILE → FOUNDING_CHAIN → BUY_STOCKS)
- [ ] Tests for game end from various phases
- [ ] All tests pass
