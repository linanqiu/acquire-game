...
- [x] `SessionManager.Room` stores `game: Game` instead of `game: dict`
- [x] All WebSocket message handlers call `Game` methods (not direct dict manipulation)
- [x] `broadcast_game_state()` uses `Game.get_public_state()` and `Game.get_player_state()`
- [x] No raw dict access to game state in `main.py` (except for serialization)
- [x] All 448 existing tests pass
- [x] No breaking changes to WebSocket message format (frontend compatibility)
