...
- [x] Connections persist for full game duration (30s ping keepalive)
- [x] Reconnection works after network interruption
- [x] No premature connection drops
- [x] Works with frontend deployed separately (via `VITE_WS_URL`; superseded — see note below)
- [x] CORS configured for WebSocket origins

> **Update (2026-07)**: Deployment was consolidated into a single container.
> `deploy/Dockerfile` now builds the frontend and the backend serves it from
> the same origin (see `docs/deployment.md`), so the frontend is **no longer
> deployed separately**. Same-origin relative WebSocket URLs are used
> (`VITE_WS_URL` unset) and CORS is effectively moot in production.
