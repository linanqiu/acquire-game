...
class MCTSBot:
    ...
    def _simulate(self, game: Game, player_id: str, action: Action) -> float:
        ...
        while not sim_game.is_game_over():
            current_player = sim_game.get_current_player_id()
            actions = Rules.get_all_legal_actions(sim_game, current_player)

            if not actions:
                break

            random_action = self.rng.choice(actions)
            sim_game.apply_action(random_action)

        # Score: 1 for win, 0.5 for tie, 0 for loss
        rankings = sim_game.get_final_rankings()
        my_rank = next(
