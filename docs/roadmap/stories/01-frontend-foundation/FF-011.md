...
- [x] `npm run e2e` starts both frontend (5173) and backend (8000) automatically
- [x] E2E tests can create rooms, add bots, and play through game scenarios
- [x] Frontend types match backend WebSocket message schemas
- [x] Helper utilities exist for: `createRoom`, `joinRoom`, `addBot`, `startGame`, `waitForPhase`, `playTile`
- [x] At least one E2E test plays a complete game with human + bot players
- [ ] Tests are deterministic (same seed = same game outcome) - Note: Backend seeding not yet implemented
