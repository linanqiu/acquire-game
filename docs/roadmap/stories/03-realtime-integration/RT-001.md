...
- [ ] Connect to `ws://host/ws/player/{room}/{token}`
...
ws.connect('ws://localhost:8000/ws/player/ABCD/token123')
ws.send({ type: 'play_tile', coordinate: '3B' })
```

### Backend WebSocket Endpoints
From `backend/main.py`:
- `/ws/player/{room_code}/{token}` - Player WebSocket
- `/ws/host/{room_code}` - Host WebSocket
