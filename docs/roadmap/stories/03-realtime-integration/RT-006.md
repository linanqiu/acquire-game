...
- `categorizeError()` - categorizes any error into: game_rule, validation, connection, server, timeout, unknown
- `ERROR_MESSAGE_MAP` - maps backend error codes to user-friendly messages
- `ERROR_CATEGORY_MAP` - maps known error keys to their categories
- `getToastType()` - returns 'warning' for game_rule/validation, 'error' for others
- `logError()` - logs errors with context for debugging

**Error Handler Hook**: `frontend/src/hooks/useErrorHandler.ts`
- `handleError()` - categorizes, shows toast, logs
- `handleServerError()` - for WebSocket error messages
- `handleConnectionError()` - for connection issues

**Integration**:
- `PlayerPage.tsx` and `HostPage.tsx` use `useErrorHandler` for WebSocket errors
- 28 unit tests cover categorization, toast types, and logging
