...
- File at `store/` not `stores/`
- Message handling via `handleMessage` method that dispatches based on message type
- Types aligned with actual backend API (`GameStateMessage`, `LobbyUpdateMessage`, etc.)
- `currentPlayer` tracks the local player's info, `gameState.current_player` tracks whose turn it is
