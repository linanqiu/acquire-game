...
- [ ] E2E test creates room via UI and verifies redirect to `/play/{CODE}?is_host=1`
- [ ] Test runs successfully with `npm run e2e` in frontend/
- [ ] Test uses existing `data-testid` selectors (no component changes needed)
- [ ] CI pipeline passes with E2E tests enabled
- [ ] Documentation updated with "how to run E2E tests locally"
