...
```python
{
    ...
    "permanent": bool | None,
    "would_trigger_merger": bool  # Added to detect merger scenarios
}
```
...
**Transformation function for RT-002 game store**:
```typescript
function backendPlayabilityToFrontend(playability: {
  playable: boolean
  reason: string | null
  ...
  return playability.would_trigger_merger ? 'merger' : 'playable'
}
```
