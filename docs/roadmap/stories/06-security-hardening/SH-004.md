...
- [x] Player names limited to 2-20 characters
- [x] HTML special characters escaped or stripped
- [x] Only alphanumeric, spaces, and basic punctuation allowed
- [x] Leading/trailing whitespace trimmed
- [x] Empty or whitespace-only names rejected
- [x] Tests verify sanitization rules
...
- Implemented as a plain `sanitize_player_name()` utility in `main.py` (the
  story's "Alternative" option) applied to `/create` and `/join`, returning
  HTTP 400 with the validation message on failure.
- Whitelist is ASCII-only (`[A-Za-z0-9 ._-]`) rather than the suggested
  Unicode-aware `\w` regex: rejecting non-ASCII entirely prevents homograph
  impersonation, which the `\w` approach would not.
- `html.escape()` was NOT applied: dangerous characters are rejected outright,
  so escaping would only corrupt stored names (e.g. `&` -> `&amp;`).
- Order of checks: whitespace normalization, min length, max length, charset -
  so an over-long XSS payload reports the length error first (still rejected).
- Frontend `validateName` in `LobbyPage.tsx` mirrors the rules (min 2 chars +
  same pattern) for defense in depth; backend remains authoritative.
- Tests live in `backend/tests/test_input_validation.py`.
